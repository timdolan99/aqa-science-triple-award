...
# --- Disciplinary Rewrite (Layman Transformation) Helpers ---
def generate_layman_transformation_prompt(
    sub_topic: str, course_title: str, level: str
) -> dict:
  """Generates a 4-mark short-answer question alongside an intentionally casual/layman draft response."""
  context = get_context(sub_topic, sub_topic)
  prompt = f"""You are an expert {course_title} ({level}) exam writer.
Topic Focus: {sub_topic}
Syllabus Context:
{context}

Task:
1. Create a 4-mark GCSE level short-answer question on this topic.
2. Write a sample student response that is factually roughly correct, BUT written entirely in casual, non-scientific 'layman' language (avoiding precise technical terminology).

Return ONLY a single valid JSON object with no additional markdown formatting:
{{
  "question": "Question text...",
  "layman_answer": "Casual draft text..."
}}"""
...
def grade_disciplinary_rewrite(
    sub_topic: str,
    question: str,
    layman_answer: str,
    student_rewrite: str,
  ...
  """Evaluates the student's rewritten answer specifically for disciplinary literacy and scientific terminology."""
  context = get_context(sub_topic, sub_topic)
  prompt = f"""You are a strict {course_title} ({level}) Senior Examiner evaluating Disciplinary Literacy.
Topic Focus: {sub_topic}
Syllabus Context:
{context}

Question: {question}
Original Layman Response: {layman_answer}
Student Scientific Rewrite: {student_rewrite}

Evaluate the student's rewrite based on:
1. Replacement of informal words with precise GCSE scientific vocabulary.
2. Clarity, logical flow, and academic scientific tone.
3. Accuracy of the core biological/scientific concepts.

Return your evaluation strictly as a single JSON object:
{{
  "score": 4,
  "max_score": 4,
  "key_terms_used": ["Term 1", "Term 2"],
  "missed_terms": ["Term 3", "Term 4"],
  "feedback": "2-3 sentences of direct, encouraging feedback on their scientific upgrade..."
}}"""
