...
- If the student attempts to switch to an unrelated or different topic (e.g., asking about 'mitosis' or 'chemical bonding' when the target topic is 'Electricity'):
  1. Politely acknowledge their input.
  2. Clarify that today's revision focus is strictly on **{sub_topic}**.
  3. Pivot the conversation back by connecting their comment to **{sub_topic}** (if a logical link exists) OR explicitly redirecting them with a probing question about **{sub_topic}**.

TUTORING MANDATE:
- Guide the student step-by-step using probing questions and constructive hints. 
- Focus on developing their disciplinary literacy as a Scientist: encourage precise physical/chemical/biological terminology, cause-and-effect mechanisms, process sequences, and core empirical equations/facts. 
...
1. Base accuracy strictly on exact scientific specification keywords derived from the Syllabus Context.
2. Ignore Setup Words: Do not count the initial topic name chosen by the student as a keyword hit.
3. Strict Terminology: Only credit official domain terms (e.g., 'transpiration', 'osmosis', 'exothermic', 'potential difference', named equations). Layperson words get 0% keyword credit.
...
2. **Performance Assessment:** Apply the rubric above, list technical scientific terms used well vs. missed across the dialogue, give 1–2 targeted feedback points, and recommend next sub-topics.
...
Generate exactly 10 short-answer exam questions testing precise definitions, cause-and-effect processes, and core scientific facts for this subtopic.
...
- Award 1 mark per question ONLY if exact scientific terms or core facts are present. Layperson terms get 0 marks.
...
def generate_extended_question(sub_topic: str, course_title: str = COURSE_TITLE, level: str = LEVEL) -> str:
    prompt = f"""You are an expert {level} Science lead examiner for {course_title}.
Create ONE high-tier 6-mark extended response question for the subtopic: '{sub_topic}'.
Use standard command words like 'Explain', 'Describe', 'Evaluate', or 'Compare'.
...
def grade_extended_response(sub_topic: str, question: str, student_answer: str, course_title: str = COURSE_TITLE, level: str = LEVEL) -> Dict[str, Any]:
    prompt = f"""You are a senior {level} Science examiner evaluating an extended response.
Topic: {sub_topic}
Question: {question}
Student Response: {student_answer}

Evaluate based on scientific disciplinary literacy: cause-and-effect reasoning, physical/chemical/biological mechanisms, and precise terminology.
...
def generate_layman_transformation_prompt(sub_topic: str, course_title: str = COURSE_TITLE, level: str = LEVEL) -> Dict[str, str]:
    prompt = f"""Create an exercise for {level} Science on '{sub_topic}'.
1. Write a typical 4-mark exam question.
2. Write a poorly phrased, informal student response using vague everyday language (e.g., 'gets really hot', 'makes lots of stuff', 'sucks up water' without scientific terms or equations).
...
def grade_disciplinary_rewrite(sub_topic: str, question: str, layman_answer: str, student_rewrite: str, course_title: str = COURSE_TITLE, level: str = LEVEL) -> Dict[str, Any]:
    prompt = f"""Grade a student's upgraded response in {level} Science.
Topic: {sub_topic}
Question: {question}
Original Informal Draft: {layman_answer}
Student Upgraded Rewrite: {student_rewrite}

Assess whether the student replaced informal language with precise scientific terminology, cause-and-effect mechanisms, or key formulas/facts.
